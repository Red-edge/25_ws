...
class WaypointNavigator(Node):
    ...
    def __init__(self):
        ...
        self.waypoints: List[Tuple[float, float, float]] = [
            ...
            # (3.838, -20.318, 0.857),
            # (2.489, -20.126, -0.671)
            (2.250,-19.900,-0.002),    #starting point
            (4.534, -22.262,0.001),		#first
            (6.382,-19.826,-0.002), 		#second (pick)
            (4.599,- 18.400,-0.001),        #third (place)
            (2.250,-19.900,-0.002),   #starting point
        ]
        ...
        # ========== 1.2) Pick Node Setting =======
        # ========== 1.2) Pick Node 双向通信设置 =======
        # 标记哪些航点是操作点（索引从0开始）
        # A. 发布者：向 PickPlace 节点发送命令
        self.waiting_for_pick_place = False  # 是否在等待 Pick 或 Place 操作完成
...
# 在这里进行控制
    ...
    def result_callback(self, future):
        ...
        if status == 4:  # SUCCESS
            ...
            self.get_logger().warn(f"Goal #{completed_index + 1} failed or canceled (status={status}). Proceeding to next...")

        # Only advance if not an operation point (or if operation already handled)
        self.current_index += 1
        self.send_next_goal()
    ...
    def navigation_guard_timer(self):
        ...
        #     if ev == "STOP_SIGN" and 0.0 < dist <= 0.7:
        ...
        # if self.stop_sign_state == "HOLDING":
        #     # 停车阶段：如果检测到目标，重置3s计时；否则检查计时结束
        #     if ev == "STOP_SIGN" and 0.0 < dist <= 1.0:
        #         # 目标还在，重置计时
        #         self.stop_sign_hold_end_time = now + 3.0
        #         self.get_logger().debug(f"[STOP_SIGN] Target still close ({dist:.2f} m), refresh 3s timer")
        #     else:
        #         # 目标丢失，继续计时
        #         self.get_logger().debug(f"[STOP_SIGN] Target missed ({dist:.2f} m), timer remaining: {self.stop_sign_hold_end_time - now:.2f}s")
    ...
    # ================= STOP EVENT 逻辑 (STOP_SIGN / RED 统一) =================
        ...
        if (ev == "STOP_SIGN" or ev == "RED") and 0.0 < dist <= 2.2:
    ...
    def send_place_command(self):
        """发送 '到达放置点，启动Place' 命令"""
        msg = PickPlaceEvent()
        msg.status = 3                 # 到达放置点，Nav关，Place开
  
        self.operation_pub.publish(msg)

    def pick_status_callback(self, msg: PickPlaceEvent):
        status_map = {
            0: "NAVIGATING (not arrived)",
            1: "ARRIVED_AT_PICK_POINT – NAV PAUSED, PICK STARTED",
            2: "PICK COMPLETED – RESUMING NAVIGATION",
            3: "ARRIVED_AT_PLACE_POINT – NAV PAUSED, PLACE STARTED",
...
#  ? & or and
        if msg.status == 2 and self.current_index==2:  # Pick completed
